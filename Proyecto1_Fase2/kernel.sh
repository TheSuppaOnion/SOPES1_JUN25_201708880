...
if lsmod | grep -q "procesos_201708880"; then
    echo -e "${YELLOW}Descargando módulo procesos existente...${NC}"
    rmmod procesos_201708880
fi
...
insmod procesos_201708880.ko

# Verificar que los módulos se cargaron correctamente
if lsmod | grep -q "cpu_201708880" && lsmod | grep -q "ram_201708880" && lsmod | grep -q "procesos_201708880" && lsmod; then
...
if [ -f /proc/cpu_201708880 ] && [ -f /proc/ram_201708880 ] && [ -f /proc/procesos_201708880 ]; then
    ...
    echo -e "${YELLOW}Datos de procesos:${NC}"
